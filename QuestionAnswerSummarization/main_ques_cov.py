...
class Summarizer(object):
    ...
    def decode(self, test_data):
        ...
        f= open("answers_cov_ques_simple_decode.txt","w")
        g= open("answers_cov_ques_original_decode.txt","w")
        ...
        for idx in range(len(test_data)):
            ex = test_data[idx]
            #ind_ques = torch.from_numpy(np.array(ex.label)).to(device)
            ind_ques = ex.label
            ...
            attention_weights =  torch.autograd.Variable(torch.FloatTensor([0]*len(all_test_input_data[idx]))).to(self.device)
            coverage_vec = torch.autograd.Variable(torch.FloatTensor([0]*len(all_test_input_data[idx]))).to(self.device)

            sos_embed = self.model_output_emb.forward(torch.tensor(sos_idx).unsqueeze(0).to(self.device))
            input_with_aux = torch.cat([ind_ques, sos_embed], dim=1)
            #pred, hidden = self.model_dec.forward(input_with_aux.unsqueeze(0), context, enc_word, coverage_vec, attention_weights)
            
            pred, hidden, coverage_vec, attention_weights = self.model_dec.forward(input_with_aux.unsqueeze(0), context, enc_word, coverage_vec, attention_weights)
            ...
            while out_idx!=eos_idx and length < self.output_max_len:
                ...
                input_with_aux = torch.cat([ind_ques, word_embed], dim=1)
                pred, hidden,coverage_vec, attention_weights = self.model_dec.forward(input_with_aux.unsqueeze(0), hidden, enc_word, coverage_vec, attention_weights)
                out_idx = torch.max(pred.squeeze(0), dim = 1)[1]
                length += 1
            print(output_seq)
    ...
    def beamsearch_decode(self, test_data):
        ...
        self.model_input_emb.eval()
        self.model_output_emb.eval()
        f= open("answers_cov_ques_beamsearch.txt","w")
        g= open("answers_cov_ques_original_decode.txt","w")
        beam_size = 9
        sos_idx = self.output_indexer.index_of(SOS_SYMBOL)
        eos_idx = self.output_indexer.index_of(EOS_SYMBOL)
        ...
        for idx in range(len(test_data)):#
            ...
            while length < self.output_max_len and len(results) < beam_size:
                ...
                for h in hyps:
                    ...
                    input_with_aux = torch.cat([ind_ques, word_emb], dim=1)
...
def train_model_encdec_cov(train_data, test_data, input_indexer, output_indexer, args, device):
    ...
    model_dec = LSTMQuesCovClassifier(args.output_dim, ques_dim, args.hidden_size, len(output_indexer), input_max_len).to(device)
    ...
    for epoch in range(0,60):#20
        ...
        for idx in perm0:
            ...
            #ques_embed  = train_data[idx].label
            #ind_ques = torch.from_numpy(np.array(train_data[idx].label))
            ind_ques = train_data[idx].label
            ...
            #print(len(all_train_input_data[idx]))
            
            attention_weights =  torch.autograd.Variable(torch.FloatTensor([0]*len(all_train_input_data[idx]))).to(device)
            coverage_vec = torch.autograd.Variable(torch.FloatTensor([0]*len(all_train_input_data[idx]))).to(device)

            if True:
                for tgt_idx in train_data[idx].best_x:
                    #ques_embed = model_ques_enc.forward(ind_ques)
                    ...
                    pred, hidden, coverage_vec, attention_weights = model_dec.forward(input_with_aux.unsqueeze(0), hidden, enc_word, coverage_vec, attention_weights)
        ...
        if(epoch%2==0):
            print("epoch",epoch)
            path_to_input_emb = "./input_emb_ques_cov"+str(epoch)+".pth"
            path_to_output_emb = "./output_emb_ques_cov"+str(epoch)+".pth"
            path_to_enc = "./enc_ques_cov"+str(epoch)+".pth"
            path_to_dec = "./dec_ques_cov"+str(epoch)+".pth"
            print("saved model to path",  path_to_input_emb)
    ...
    path_to_input_emb = "./input_emb_ques_cov.pth" 
    path_to_output_emb = "./output_emb_ques_cov.pth"
    path_to_enc = "./enc_ques_cov.pth"
    path_to_dec = "./dec_ques_cov.pth"
    torch.save(model_input_emb.state_dict(), path_to_input_emb)
    torch.save(model_output_emb.state_dict(), path_to_output_emb)
    torch.save(model_enc.state_dict(), path_to_enc)
    torch.save(model_dec.state_dict(), path_to_dec)
    ...
    return Summarizer(input_indexer, output_indexer, model_enc,model_dec,model_input_emb,model_output_emb,args.reverse_input,median, device, input_max_len)
...
if __name__ == '__main__':
   ...
   # print(len(answer_exs))
    ...
    ques_dim = 100
    model_ques_enc = BiLSTMEncoderQuora(device, embedding_dim=args.input_dim,hidden_dim=ques_dim,
                           vocab_size=17615)
    model_ques_enc.to(device)
    model_ques_enc_pre_train = './mr_best_model_acc_0.bilstm.model'
    model_ques_enc.load_state_dict(torch.load(model_ques_enc_pre_train, device))
    model_ques_enc.eval()
    count = 0
    for ex in answer_exs:
        ...
        with torch.no_grad():
            ind_ques = torch.from_numpy(np.array(ex.label)).to(device)
            embed_ques = model_ques_enc.forward(ind_ques)
            #embed_ques.detach().numpy()
            label = torch.from_numpy(np.array(embed_ques.cpu().numpy())).to(device)
            ex.label = label
            #06print(type(label))
            #ex.label = embed_ques.cpu().numpy()
            #torch.from_numpy(np.array(embed_ques)).to(device)
