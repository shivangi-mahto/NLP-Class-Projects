...
def train_classifier(train_exs,  device):
    ...
    for epoch in range(1):
        ...
        perm0 = np.arange(len(train_exs[:-400]))
        random.shuffle(perm0)
        for idx in perm0: #[1:10]
            ex = train_exs[idx]
            ex_loss = torch.autograd.Variable(torch.FloatTensor([0]))
            optimizer.zero_grad()
            label = [ex.ID]
            ...
            #mean = input_vec.float().mean(dim=0)
            #std = input_vec.float().std(dim=0)
...
def train_model_encdec(train_data, test_data, indexer, args, device):
   ...
   # print("Train output length: %i" % np.max(np.asarray([len(ex.best_x) for ex in train_data])))
    ...
    model_enc = BiLSTMEncoder(300, 200, 0) #, args.bidirectional)
    model_enc.to(device)
