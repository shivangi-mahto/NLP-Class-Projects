...
if __name__ == '__main__':
    ...
    elif system_to_run == "SVM":
        svm_model = train_svm_model(train)
        print("Data reading and training took %f seconds" % (time.time() - start_time))
        dev_decoded = [svm_model.decode(test_ex) for test_ex in dev]
        if args.run_on_test:
            print("Running on test")
            test = read_data(args.blind_test_path)
            test_decoded = [svm_model.decode(test_ex) for test_ex in test]
            print_output(test_decoded, args.test_output_path)
    else:
