...
#Expextec results for SVM with training epoch 5
    #Labeled F1: 84.12, precision: 4954/5836 = 84.89, recall: 4954/5943 = 83.36
    ...
    def decode(self, sentence):
        ...
        for i in range(1,len(sentence)):
            for s in range(0,len(self.tag_indexer)):
                ...
                temp_score = 1*np.zeros(len(self.tag_indexer))
                for prev_tag in range(0,len(self.tag_indexer)):
                  temp_score[prev_tag] =  Scorer.score_emission(emission_feat) + score[i-1][prev_tag] + self.transition_counts[prev_tag,s]
...
def train_svm_model(sentences):
    ...
    for epoch in range(0,8):
        for sentence_idx in range(0,len(sentences)): #len(sentences)
            ...
            bio_tags = sentence.get_bio_tags()
            tag_idx = tag_indexer.get_index(bio_tags[0])
            
            for s in range(0,len(tag_indexer)):
                emission_feat = feature_cache[sentence_idx][0][s]
                alpha[0][s] = Scorer.score_emission(emission_feat) + 1 - int(np.equal(s,tag_idx)) #wTfe #alpha here are scores - forward counts only            
            ...
            for i in range(1,len(sentence)):     
                ...
                for s in range(0,len(tag_indexer)):
                    ...
                    for prev_tag in range(0,len(tag_indexer)):
                        temp_alpha[prev_tag] = Scorer.score_emission(emission_feat) + 1 - int(np.equal(s,tag_idx))  + alpha[i-1][prev_tag] + Probability.score_transition(sentence,prev_tag,s) 
                    alpha[i][s]= np.max(temp_alpha)
                    prev_tag_max[i][s] = np.argmax(temp_alpha) 

            pred_tag_ind = np.empty(len(sentence),dtype=int)

            pred_tag_ind[len(sentence)-1] = np.argmax(alpha[len(sentence)-1])
            for i in range(1,len(sentence)):
                pred_tag_ind[len(sentence)-i-1] = prev_tag_max[len(sentence)-i][pred_tag_ind[len(sentence)-i]]
    ...
    return SVMModel(tag_indexer, feature_indexer, weights, hmm.transition_log_probs)
